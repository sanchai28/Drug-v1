...
requisitions_bp = Blueprint('requisitions_bp', __name__, url_prefix='/api/requisitions')
...
@requisitions_bp.route('/<int:requisition_id>', methods=['GET'])
...
@requisitions_bp.route('/pending_approval', methods=['GET'])
...
@requisitions_bp.route('/<int:requisition_id>/items', methods=['GET'])
...
@requisitions_bp.route('/<int:requisition_id>/cancel', methods=['PUT'])
def cancel_requisition_endpoint(requisition_id):
    ...
    try:
        ...
        current_app.logger.error(f"Database error during requisition hard delete: {e}", exc_info=True)
        return jsonify({"error": f"เกิดข้อผิดพลาดในการลบใบเบิก: {e}"}), 500
    except Exception as ex:
        if conn: conn.rollback()
        current_app.logger.error(f"General error during requisition hard delete: {ex}", exc_info=True)
        return jsonify({"error": f"เกิดข้อผิดพลาดทั่วไป: {ex}"}), 500
    finally:
        if cursor: cursor.close()
        if conn: conn.close()

@requisitions_bp.route('/<int:requisition_id>/process_approval', methods=['PUT'])
def process_requisition_approval(requisition_id):
    ...
    try:
        ...
        if conn: conn.rollback()
        current_app.logger.error(f"Database error during requisition approval: {e}", exc_info=True)
        return jsonify({"error": f"เกิดข้อผิดพลาดในการดำเนินการใบเบิก: {e}"}), 500
    except Exception as ex:
        if conn: conn.rollback()
        current_app.logger.error(f"General error during requisition approval: {ex}", exc_info=True)
        return jsonify({"error": f"เกิดข้อผิดพลาดทั่วไป: {ex}"}), 500
    finally:
        if cursor: cursor.close()
        if conn: conn.close()
...
def create_requisition():
    ...
    try:
        ...
        if conn: conn.rollback()
        current_app.logger.error(f"Database error during requisition creation: {e}", exc_info=True)
        return jsonify({"error": f"Database error: {e}"}), 500
    except Exception as ex:
        if conn: conn.rollback()
        current_app.logger.error(f"General error during requisition creation: {ex}", exc_info=True)
